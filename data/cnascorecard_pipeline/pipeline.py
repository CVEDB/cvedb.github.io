...
from .badge_generator import generate_badge_documentation, generate_cna_badges
from .chunking import generate_search_index, write_chunked_cna_data
from .completeness import compute_field_utilization, compute_individual_cna_field_utilization
from .config import ANALYSIS_PERIOD_MONTHS, get_config
from .ingest import get_date_range_for_period, load_cve_records, load_delta_cves, validate_date_range
...
from .utils import ProgressTracker, ensure_directory_exists, setup_logging, write_json_file
...
class CNAScoreCardPipeline:
    ...
    def _sanitize_cna_filename_part(value: str) -> str:
        ...
        if not value:
            return ""
        return "".join(c for c in str(value) if c.isalnum() or c in ("-", "_", ".")).rstrip()
    ...
    def _setup_analysis_periods(self, start_date: str | None, end_date: str | None) -> None:
        """
        ...
        self.logger.info(
            f"Previous analysis period: {previous_start.strftime('%Y-%m-%d')} to {previous_end.strftime('%Y-%m-%d')}"
        )
    ...
    def _aggregate_cna_scores(self) -> None:
        ...
        current_period = tuple(datetime.strptime(date, "%Y-%m-%d") for date in self.analysis_periods["current"])
        previous_period = tuple(datetime.strptime(date, "%Y-%m-%d") for date in self.analysis_periods["previous"])
    ...
    def _generate_completeness_analysis(self) -> None:
        ...
        self.field_utilization = compute_field_utilization(self.filtered_cve_records, field_list)

        # Generate individual CNA field utilization
        self.individual_cna_utilization = compute_individual_cna_field_utilization(
            self.filtered_cve_records, field_list
        )
    ...
    def _generate_trends_analysis(self) -> None:
        ...
        self.top_improvers = calculate_top_improvers(
            self.cve_records, Path(self.config["directories"]["web_data"]), top_n=10
        )
    ...
    def _generate_output_files(self, output_dir: str | None = None) -> dict[str, str]:
        """
    ...
    def _generate_individual_cna_files(self, output_dir: Path) -> dict[str, str]:
        ...
        if cna_list_path.exists():
            with open(cna_list_path) as f:
        ...
        active_cna_count = sum(
            1 for cna_data in self.cna_outputs.values() if cna_data["cna_scoring"][0].get("recent_cves_count", 0) > 0
        )
        ...
        for cna_name in self.cna_outputs:
        ...
        for cna_name, cna_data in self.cna_outputs.items():
            try:
                ...
                filename = self._build_cna_detail_filename(cna_name, official_meta.get("cnaID", ""))
                ...
                if "cve_scoring" in cna_data and cna_data["cve_scoring"]:
                    for cve_data in cna_data["cve_scoring"]:
                        ...
                        recent_cve = {
                            ...
                            "scoreBreakdown": {
                                "foundationalCompleteness": cve_data.get("scoreBreakdown", {}).get(
                                    "foundationalCompleteness", 0
                                ),
                                "rootCauseAnalysis": cve_data.get("scoreBreakdown", {}).get("rootCauseAnalysis", 0),
                                "severityAndImpactContext": cve_data.get("scoreBreakdown", {}).get(
                                    "severityAndImpactContext", 0
                                ),
                                "softwareIdentification": cve_data.get("scoreBreakdown", {}).get(
    ...
    def _generate_summary_files(self, output_dir: Path) -> dict[str, str]:
        ...
        for i, (cna_name, cna_data, score) in enumerate(sorted_cnas):
            ...
            web_cna = {
                "shortName": cna_name,
                "detailFile": self._build_cna_detail_filename(cna_name, official_meta.get("cnaID", "")),
                "organizationName": official_meta.get("organizationName", cna_name),
    ...
    def _generate_completeness_files(self, output_dir: Path) -> dict[str, str]:
        ...
        for cna_name, utilization_data in self.individual_cna_utilization.items():
            safe_filename = "".join(c for c in cna_name if c.isalnum() or c in ("-", "_", ".")).rstrip()
    ...
    def _generate_cna_badges(self, output_dir: Path) -> dict[str, str]:
        ...
        base_url = self.config.get("website", {}).get("base_url", "https://cvedb.github.io")
...
Examples:
  ...
  python pipeline.py --no-cache                # Disable score caching
    ...
    parser.add_argument(
        "--delta", action="store_true", help="Use delta mode for incremental processing (only process changed CVEs)"
    )
    parser.add_argument("--no-cache", action="store_true", help="Disable score caching (useful for debugging)")
