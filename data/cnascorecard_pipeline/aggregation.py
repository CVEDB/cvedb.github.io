...
from .config import CNA_LIST_FILE
...
def aggregate_cna_scores(scored_cves: list[dict], periods: list[tuple[datetime, datetime]]) -> dict[str, Any]:
    ...
    def summarize_trend(monthly_trends: list) -> dict[str, str]:
        ...
        return {"trend_direction": "steady", "trend_description": "No significant change"}
    ...
    def map_cve_name_to_official(cve_name: str, official_names: set, metadata_map: dict[str, Any]) -> str | None:
    ...
    for official_cna_name in official_cna_names:
        ...
        if official_cna_name.lower() == "fortinet":
            logger.debug(
                "Fortinet patchinfo avg: %s (sum: %s, count: %d)",
                cat_avgs.get("patchinfo", 0),
                cat_sums.get("patchinfo", 0),
                len(recent_cves),
            )
