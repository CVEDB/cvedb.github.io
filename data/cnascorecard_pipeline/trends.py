...
def calculate_daily_trends(
    cve_records: list[dict[str, Any]], output_dir: Path, analysis_days: int = 180
) -> dict[str, Any]:
    """
    ...
    logger.info(
        f"Trends analysis stats: {parsed_count} had dates, {valid_date_count} parsed successfully, {in_window_count} within analysis window"
    )
...
def calculate_top_improvers(
    cve_records: list[dict[str, Any]], output_dir: Path, top_n: int = 10
) -> list[dict[str, Any]]:
    """
    ...
    for cna_name, daily_data in cna_daily_scores.items():
        ...
        for category in ["root_cause_analysis", "severity_and_impact", "software_identification", "patch_information"]:
    ...
    improvers_data = {
        ...
        "generated_at": datetime.now().isoformat(),
        "analysis_criteria": {"minimum_cves": 5, "top_n_count": top_n},
    }
...
def _calculate_daily_category_averages(day_cves: list[dict[str, Any]]) -> dict[str, float]:
    ...
    averages = {}
    for category in ["rootCauseAnalysis", "severityAndImpactContext", "softwareIdentification", "patchinfo"]:
...
def _calculate_rolling_averages(daily_trends: dict[str, dict], window_days: int = 7) -> dict[str, dict]:
...
def _get_period_average_scores(daily_data: dict[str, list], date_list: list[str]) -> dict[str, float] | None:
