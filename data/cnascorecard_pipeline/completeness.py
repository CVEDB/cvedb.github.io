...
def _custom_check(data: Any, check_type: str) -> bool:
    ...
    if check_type == "multiple_languages":
        if isinstance(data, list):
            languages = {desc.get("lang", "") for desc in data if isinstance(desc, dict)}
    ...
    if check_type == "has_advisory_ref":
        if isinstance(data, list):
            return any(
                ref.get("tags") and any("advisory" in str(tag).lower() for tag in ref["tags"])
                for ref in data
    ...
    if check_type == "has_patch_ref":
        if isinstance(data, list):
            return any(
                ref.get("tags") and any("patch" in str(tag).lower() for tag in ref["tags"])
                for ref in data
                if isinstance(ref, dict)
            )
        return False
    if check_type == "has_exploit_ref":
        if isinstance(data, list):
            return any(
                ref.get("tags") and any("exploit" in str(tag).lower() for tag in ref["tags"])
                for ref in data
                if isinstance(ref, dict)
            )
        return False
    if check_type == "has_technical_ref":
        if isinstance(data, list):
            return any(
                ref.get("tags") and any("technical" in str(tag).lower() for tag in ref["tags"])
                for ref in data
                if isinstance(ref, dict)
            )
        return False
    if check_type == "has_vendor_ref":
        if isinstance(data, list):
            return any(
                ref.get("tags") and any("vendor" in str(tag).lower() for tag in ref["tags"])
                for ref in data
                if isinstance(ref, dict)
            )
        return False
    ...
    if check_type == "has_other_metrics":
        if isinstance(data, list):
            return any(
                any(k not in ("cvssV4_0", "cvssV3_1", "cvssV3_0", "cvssV2_0") for k in metric)
                for metric in data
...
def compute_field_utilization(cve_records: list[dict[str, Any]], field_list: list[str]) -> list[dict[str, Any]]:
    ...
    for field in field_list:
        ...
        utilization.append({"field": field, "unique_cnas": unique_cnas, "cna_percent": cna_percent})
    # Sort by cna_percent descending, then field name ascending
    utilization.sort(key=lambda x: (-x["cna_percent"], x["field"]))
    return utilization


def compute_individual_cna_field_utilization(
    cve_records: list[dict[str, Any]], field_list: list[str]
