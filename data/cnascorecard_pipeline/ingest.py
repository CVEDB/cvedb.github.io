...
from .utils import ProgressTracker, extract_cna_short_name, load_json_file, validate_cve_record
...
def load_cve_records(
    cve_dir: str | None = None, start_date: str | None = None, end_date: str | None = None
) -> list[dict[str, Any]]:
    """
...
def _get_cve_file_list(cve_dir: str, start_date: str | None, end_date: str | None) -> list[str]:
    """
...
def _get_year_directories(cves_dir: Path) -> list[str]:
    ...
    """
    try:
        year_dirs = [d.name for d in cves_dir.iterdir() if d.is_dir() and d.name.isdigit()]
...
def _get_year_files(cves_dir: Path, year: str) -> list[str]:
    ...
    """
    return list(cves_dir.glob(f"{year}/**/CVE-*.json"))
...
def _load_cves_sequential(
    cve_files: list[str], start_dt: datetime | None, end_dt: datetime | None
) -> list[dict[str, Any]]:
...
def _load_cves_parallel(
    cve_files: list[str], start_dt: datetime | None, end_dt: datetime | None, max_workers: int | None = None
) -> list[dict[str, Any]]:
    """
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(_load_cve_batch, batch, start_str, end_str): i for i, batch in enumerate(batches)}
...
def _load_cve_batch(
    file_paths: list[str], start_date_str: str | None, end_date_str: str | None
...
def _load_single_cve_file(file_path: str) -> dict[str, Any] | None:
    """
...
def _passes_date_filter(cve: dict[str, Any], start_dt: datetime | None, end_dt: datetime | None) -> bool:
    """
    ...
    try:
        ...
        return not (end_dt and pub_dt > end_dt)
...
def load_delta_cves(
    cve_dir: str | None = None, since: str | None = None
    ...
    new_records = []
    updated_records = []
    metadata = {"fetchTime": None, "numberOfChanges": 0, "errors": []}
...
def _cve_id_to_path(cves_dir: Path, cve_id: str) -> Path | None:
    """
...
def get_last_run_timestamp(state_file: Path) -> str | None:
    """
...
def save_run_timestamp(state_file: Path, timestamp: str | None = None) -> None:
    """
    ...
    state = {"last_run_timestamp": timestamp, "updated_at": datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%S.%fZ")}
