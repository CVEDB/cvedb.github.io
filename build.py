...
app = typer.Typer(
    name="cve-icu",
    help="CVEDB Static Site Generator - Build and manage vulnerability intelligence site.",
    add_completion=True,
...
class CVESiteBuilder:
    ...
    def __init__(self, quiet: bool = False) -> None:
        ...
        self.published_totals_url: str = "https://cvedb.github.io/data/cve_all.json"
        ...
        if not self.quiet:
            logger.info("🚀 CVEDB Build System Initialized")
    ...
    def ensure_scorecard_subsite(self) -> None:
        """Preserve the committed CNA Scorecard sub-site in staged builds.

        The scorecard pages and their last generated data live in the committed
        web/scorecard directory (regenerated by the scorecard workflow, see
        .github/workflows/cna-scorecard.yml). A clean build replaces web/
        wholesale, so copy the committed sub-site into staging before publish.
        """
        source = self.output_root_dir / "scorecard"
        destination = self.web_dir / "scorecard"
        if not source.exists():
            self.print_verbose("  ⚠️  Scorecard sub-site not found, skipping")
            return
        shutil.copytree(source, destination)
        self.print_verbose("  ✅ Copied scorecard sub-site into staged output")
    ...
    def generate_html_pages(self) -> None:
        ...
        pages = [
            ...
            {"template": "about.html", "output": "about.html", "title": "About CVEDB"},
        ]
    ...
    def build_site(
        ...
        try:
            ...
            # Step 2.5: Preserve the committed scorecard sub-site in staging
            self.ensure_scorecard_subsite()
...
def build(
    ...
    """Build the CVEDB static site.

    This is the main build command that generates all HTML pages
    and JSON data files for the CVEDB vulnerability intelligence site.
...
def info() -> None:
    ...
    console.print("\n[bold cyan]CVEDB Build System Info[/bold cyan]\n")
