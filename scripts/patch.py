...
def load_vendor_brand_map(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        normalized = {str(k): str(v) for k, v in loaded.items() if isinstance(k, str) and isinstance(v, str)}
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Patch Data Aggregator")
    ...
    parser.add_argument("--output", type=Path, default=Path("data/patch_data.csv"))
    parser.add_argument("--epss-threshold", type=float, default=0.95, help="EPSS score threshold (default: 0.95)")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose/debug logging")
    parser.add_argument("--dry-run", action="store_true", help="Report what would be produced without writing files")
    ...
    patch_df = pd.merge(cve_list, nvd, how="inner", left_on="CVE", right_on="CVE")

    # Fill empty vendor/product from CISA when NVD CPE parsing yields blanks.
    patch_df["vendor"] = patch_df["vendor"].fillna("")
    patch_df["product"] = patch_df["product"].fillna("")
    patch_df["cisa_vendor"] = patch_df["cisa_vendor"].fillna("")
    patch_df["cisa_product"] = patch_df["cisa_product"].fillna("")
    patch_df.loc[patch_df["vendor"].str.strip() == "", "vendor"] = patch_df["cisa_vendor"]
    patch_df.loc[patch_df["product"].str.strip() == "", "product"] = patch_df["cisa_product"]

    if not epss_df_all.empty:
        patch_df = pd.merge(patch_df, epss_df_all, how="inner", left_on="CVE", right_on="CVE")
        columns = [
            "CVE",
            "CVSS Score",
            "cvss_vector",
    ...
    logger.info(f"Final dataset: {len(patch_df)} CVEs")
    if args.dry_run:
        logger.info("Dry run mode — no files written")
        logger.info(f"Columns: {list(patch_df.columns)}")
    ...
    web_csv_path = Path("web/patch/data.csv")
    web_csv_path.parent.mkdir(parents=True, exist_ok=True)
    patch_df.to_csv(web_csv_path, index=False)
