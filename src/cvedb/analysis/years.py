...
class CVEYearsAnalyzer:
    ...
    def __init__(self, cache_dir=None, quiet=False):
        ...
        self.downloader = CVEDataDownloader(cache_dir=cache_dir, quiet=quiet)
        self.data_file = None
    ...
    def ensure_data_loaded(self):
        ...
        if self.data_file is None:
            if not self.quiet:
                print("🔽 Ensuring CVE data source is available...")
            self.data_file = self.downloader.ensure_data_available()
            if not self.quiet:
                print(f"✅ Data source ready: {self.data_file}")
        
        if not hasattr(self, 'all_cves') or self.all_cves is None:
            if not self.quiet:
                print("  📊 Reading NVD JSON data into memory...")
            with open(self.data_file, 'r', encoding='utf-8') as f:
                try:
                    self.all_cves = json.load(f)
                    if not self.quiet:
                        print(f"  ✅ Loaded {len(self.all_cves):,} CVE records")
                except json.JSONDecodeError as e:
                    print(f"  ❌ Failed to parse JSON: {e}")
                    self.all_cves = []

        # Lazy-load EPSS mapping (best-effort)
        ...
        # Lazy-load KEV mapping (best-effort)
    ...
    def _init_year_stats(self):
        """Initialize statistics counters for a year"""
        return {
            'monthly_counts': [0] * 12,
            'daily_counts': {},
            'cvss_data': {
            ...
            'epss_buckets': {'epss_gt_0_1': 0, 'epss_gt_0_5': 0, 'epss_gt_0_9': 0},
            'kev_count': 0,
            'total_cves': 0
        }

    def _update_year_stats(self, stats, cve_data, pub_date):
        """Update year statistics with data from a single CVE"""
        ...
        stats['monthly_counts'][month_idx] += 1
        ...
        cvss_target = stats['cvss_data'].get(cvss_version, stats['cvss_data']['unknown'])
        cvss_target['severity_counts'][severity] += 1
        score_key = round(float(score), 1) if score > 0 else 0.0
        cvss_target['score_distribution'][score_key] += 1
        cvss_target['total'] += 1
        
        # EPSS
        if hasattr(self, 'epss_mapping'):
            epss = self.epss_mapping.get(cve_id)
            if epss:
                epss_score = float(epss.get('epss_score') or 0.0)
                if epss_score > 0.1: stats['epss_buckets']['epss_gt_0_1'] += 1
                if epss_score > 0.5: stats['epss_buckets']['epss_gt_0_5'] += 1
                if epss_score > 0.9: stats['epss_buckets']['epss_gt_0_9'] += 1

        # KEV
        if hasattr(self, 'kev_mapping') and self.kev_mapping.get(cve_id):
            stats['kev_count'] += 1
            
        # Additional metadata
        stats['status_counts'][vuln_status] += 1
        
        for tag in cve_data.get('cve', {}).get('cveTags', []):
            tag_name = tag if isinstance(tag, str) else tag.get('tag')
            if tag_name: stats['tag_counts'][tag_name] += 1
            
        for ref in cve_data.get('cve', {}).get('references', []):
            for tag in ref.get('tags', []):
                stats['reference_tag_counts'][tag] += 1
                
        for vendor in self.extract_cpe_vendor_info(cve_data):
            stats['cpe_vendor_counts'][vendor] += 1
            
        for vendor in self.extract_vendor_info(cve_data):
            stats['vendor_counts'][vendor] += 1
            
        for cwe in self.extract_cwe_info(cve_data):
            stats['cwe_counts'][cwe] += 1

    def _finalize_year_data(self, year, stats):
        """Finalize year statistics into the final JSON structure"""
        daily_analysis = {
            'total_days': 0, 'avg_per_day': 0,
            'highest_day': {'date': '', 'count': 0},
            'lowest_day': {'date': '', 'count': 0},
            'daily_counts': self.create_complete_daily_array(stats['daily_counts'], year)
        }
        
        non_zero_days = {k: v for k, v in daily_analysis['daily_counts'].items() if v > 0}
        if non_zero_days:
            values = list(non_zero_days.values())
            daily_analysis.update({
                'total_days': len(non_zero_days),
                'avg_per_day': round(sum(values) / len(values), 2),
                'highest_day': {'date': max(non_zero_days, key=non_zero_days.get), 'count': max(values)},
                'lowest_day': {'date': min(non_zero_days, key=non_zero_days.get), 'count': min(values)}
            })
        ...
        return {
            ...
            'date_data': {
                'monthly_distribution': {str(i+1): stats['monthly_counts'][i] for i in range(12)},
                'daily_analysis': daily_analysis
            },
            'cvss': {
                **{v: {'total': d['total'], 'severity_distribution': dict(d['severity_counts']), 
                       'score_distribution': dict(d['score_distribution'])}
                   for v, d in stats['cvss_data'].items() if d['total'] > 0},
                'epss_summary': stats['epss_buckets']
            },
            'kev': {'kev_count': stats['kev_count']},
            'vendors': {
                'cna_assigners': [{'name': v, 'count': c} for v, c in stats['vendor_counts'].most_common(20)],
                'cpe_vendors': [{'name': v, 'count': c} for v, c in stats['cpe_vendor_counts'].most_common(20)]
            },
            'cwe': {'top_cwes': [{'cwe': c, 'count': count} for c, count in stats['cwe_counts'].most_common(20)]},
            'metadata': {
                'vulnerability_status': [{'status': s, 'count': c} for s, c in stats['status_counts'].most_common(10)],
                'cve_tags': [{'tag': t, 'count': c} for t, c in stats['tag_counts'].most_common(10)],
                'reference_tags': [{'tag': t, 'count': c} for t, c in stats['reference_tag_counts'].most_common(20)]
            },
            'processing_stats': {
    ...
    def process_years(self, years):
        """Process multiple years in a single pass over the CVE data"""
        self.ensure_data_loaded()
        if not self.all_cves:
            return {year: self.create_empty_year_data(year) for year in years}

        target_years = set(years)
        year_stats = {year: self._init_year_stats() for year in target_years}
        
        if not self.quiet:
            print(f"📊 Processing {len(target_years)} years in a single pass...")

        for idx, cve_data in enumerate(self.all_cves):
            if not self.quiet and idx % 25000 == 0 and idx > 0:
                print(f"  🔍 Scanning CVEs: {idx:,} / {len(self.all_cves):,}")
            ...
            if year in target_years:
                vuln_status = cve_data.get('cve', {}).get('vulnStatus', '')
                if 'Rejected' not in vuln_status:
                    self._update_year_stats(year_stats[year], cve_data, pub_date)

        for year in target_years:
            self.year_data_cache[year] = self._finalize_year_data(year, year_stats[year])
        
        return self.year_data_cache
    ...
    def process_year_data(self, year):
        ...
        return self.process_years([year])[year]
