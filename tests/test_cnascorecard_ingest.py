...
from cnascorecard_pipeline.ingest import (
    _get_cve_file_list,
...
class TestIngestFixtures:
    ...
    def sample_cve_rejected(self) -> dict[str, Any]:
        ...
        return {
            ...
            "containers": {"cna": {"providerMetadata": {"shortName": "TestCNA"}}},
        }

    @pytest.fixture
    def sample_cve_invalid(self) -> dict[str, Any]:
        """Invalid CVE record for testing."""
        return {"invalid": "data", "no_metadata": True}
...
class TestLoadCveRecords(TestIngestFixtures):
    ...
    @patch("cnascorecard_pipeline.ingest._load_and_filter_cves")
    @patch("cnascorecard_pipeline.ingest._get_cve_file_list")
    def test_load_cve_records_basic(self, mock_get_files, mock_load_filter, mock_cve_files, sample_cve_valid):
    ...
    @patch("cnascorecard_pipeline.ingest._load_and_filter_cves")
    @patch("cnascorecard_pipeline.ingest._get_cve_file_list")
    def test_load_cve_records_with_dates(self, mock_get_files, mock_load_filter, mock_cve_files, sample_cve_valid):
        """Test CVE record loading with date filtering."""
        mock_get_files.return_value = mock_cve_files
        mock_load_filter.return_value = [sample_cve_valid]

        result = load_cve_records(start_date="2024-01-01", end_date="2024-12-31")

        assert len(result) == 1
        mock_get_files.assert_called_with(str(Path(__file__).parent.parent / "cve_data"), "2024-01-01", "2024-12-31")
    ...
    @patch("cnascorecard_pipeline.ingest._load_and_filter_cves")
    @patch("cnascorecard_pipeline.ingest._get_cve_file_list")
    def test_load_cve_records_custom_dir(self, mock_get_files, mock_load_filter, sample_cve_valid):
...
class TestGetCveFileList(TestIngestFixtures):
    ...
    @patch("cnascorecard_pipeline.ingest._get_year_files")
    @patch("cnascorecard_pipeline.ingest._get_year_directories")
    @patch("pathlib.Path.exists")
    def test_get_cve_file_list_with_dates(self, mock_exists, mock_year_dirs, mock_year_files):
...
class TestGetYearFiles:
    ...
    def test_get_year_files_success(self, mock_glob):
        ...
        mock_glob.assert_called_once_with("2024/**/CVE-*.json")


class TestLoadAndFilterCves(TestIngestFixtures):
    """Test CVE loading and filtering."""

    @patch("cnascorecard_pipeline.ingest._load_single_cve_file")
    @patch("cnascorecard_pipeline.ingest._passes_date_filter")
    def test_load_and_filter_cves_success(self, mock_date_filter, mock_load_file, sample_cve_valid):
    ...
    @patch("cnascorecard_pipeline.ingest._load_single_cve_file")
    @patch("cnascorecard_pipeline.ingest._passes_date_filter")
    def test_load_and_filter_cves_with_failures(self, mock_date_filter, mock_load_file, sample_cve_valid):
...
class TestLoadSingleCveFile(TestIngestFixtures):
    ...
    @patch("cnascorecard_pipeline.ingest.load_json_file")
    @patch("cnascorecard_pipeline.ingest.validate_cve_record")
    def test_load_single_cve_file_success(self, mock_validate, mock_load_json, sample_cve_valid):
    ...
    @patch("cnascorecard_pipeline.ingest.load_json_file")
    @patch("cnascorecard_pipeline.ingest.validate_cve_record")
    def test_load_single_cve_file_invalid(self, mock_validate, mock_load_json, sample_cve_invalid):
    ...
    @patch("cnascorecard_pipeline.ingest.load_json_file")
    @patch("cnascorecard_pipeline.ingest.validate_cve_record")
    def test_load_single_cve_file_rejected(self, mock_validate, mock_load_json, sample_cve_rejected):
...
class TestPassesDateFilter(TestIngestFixtures):
    ...
    def test_passes_date_filter_invalid_date(self):
        ...
        cve_invalid_date = {"cveMetadata": {"datePublished": "invalid-date-format"}}
...
class TestLoadCnaList(TestIngestFixtures):
    ...
    def test_load_cna_list_multiple_cnas(self):
        ...
        cves = [
            {"containers": {"cna": {"providerMetadata": {"shortName": "CNA1"}}}},
            {"containers": {"cna": {"providerMetadata": {"shortName": "CNA2"}}}},
            {
                "containers": {
                    "cna": {
...
class TestIngestIntegration:
    ...
    @patch("cnascorecard_pipeline.ingest.Path.exists")
    @patch("cnascorecard_pipeline.ingest.Path.iterdir")
    @patch("pathlib.Path.glob")
    @patch("builtins.open", new_callable=mock_open)
    @patch("cnascorecard_pipeline.ingest.load_json_file")
    def test_full_integration(self, mock_load_json, mock_file, mock_glob, mock_iterdir, mock_exists):
        """Test full integration of ingest functionality."""
        # Setup mocks
        mock_exists.return_value = True
        mock_iterdir.return_value = [MagicMock(name="2024", is_dir=lambda: True)]
        ...
        # Test the full flow
        records = load_cve_records(cve_dir="/test/dir", start_date="2024-01-01", end_date="2024-12-31")
