...
import pytest
from cnascorecard_pipeline.chunking import (
    _get_timestamp,
    _grade_sort_key,
    cleanup_old_chunks,
...
class TestWriteChunkedCNAData:
    ...
    def sample_cna_list(self):
        ...
        return [{"shortName": f"CNA_{i}", "cnaId": f"id_{i}", "grade": "A" if i % 2 == 0 else "B"} for i in range(125)]
...
class TestWriteChunkedCompletenessData:
    ...
    def sample_completeness_list(self):
        ...
        return [{"cnaName": f"CNA_{i}", "completeness": 75.0 + i} for i in range(150)]
...
class TestGenerateSummaryStats:
    ...
    def test_creates_summary_stats(self, sample_cna_list, tmp_path):
        ...
        generate_summary_stats(sample_cna_list, tmp_path)
