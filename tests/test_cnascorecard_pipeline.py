...
from cnascorecard_pipeline.pipeline import CNAScoreCardPipeline, PipelineError, main
...
class TestPipelineFixtures:
    ...
    def sample_cve_records(self) -> list[dict[str, Any]]:
        ...
        return [
            {
                "cveId": "CVE-2024-0001",
                "cveMetadata": {"datePublished": "2024-01-15T10:00:00.000Z"},
                "containers": {"cna": {"providerMetadata": {"shortName": "TestCNA1"}}},
            },
            {
                "cveId": "CVE-2024-0002",
                "cveMetadata": {"datePublished": "2024-01-20T10:00:00.000Z"},
                "containers": {"cna": {"providerMetadata": {"shortName": "TestCNA2"}}},
            },
        ]
...
class TestCNAScoreCardPipeline(TestPipelineFixtures):
    ...
    def test_pipeline_initialization_default_config(self):
        ...
        with patch("cnascorecard_pipeline.pipeline.get_config") as mock_get_config:
    ...
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._generate_output_files")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._generate_trends_analysis")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._generate_completeness_analysis")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._aggregate_cna_scores")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._score_cve_records")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._load_cve_data")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._sync_cna_metadata")
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline._setup_analysis_periods")
    def test_pipeline_run_success(
        ...
        pipeline.analysis_periods = {"current": ("2024-01-01", "2024-06-30"), "previous": ("2023-07-01", "2023-12-31")}
    ...
    @patch("cnascorecard_pipeline.pipeline.get_date_range_for_period")
    ...
    @patch("cnascorecard_pipeline.pipeline.load_cve_records")
    ...
    @patch("cnascorecard_pipeline.pipeline.score_multiple_cves")
    ...
    @patch("cnascorecard_pipeline.pipeline.aggregate_cna_scores")
    ...
    @patch("cnascorecard_pipeline.pipeline.compute_individual_cna_field_utilization")
    @patch("cnascorecard_pipeline.pipeline.compute_field_utilization")
    def test_generate_completeness_analysis(self, mock_field_util, mock_individual_util, sample_config):
...
class TestPipelineOutputGeneration(TestPipelineFixtures):
    ...
    @patch("cnascorecard_pipeline.pipeline.write_json_file")
    @patch("cnascorecard_pipeline.pipeline.ensure_directory_exists")
    def test_generate_individual_cna_files(self, mock_ensure_dir, mock_write_json, sample_config, sample_cna_outputs):
    ...
    @patch("cnascorecard_pipeline.pipeline.write_json_file")
    @patch("cnascorecard_pipeline.pipeline.write_chunked_cna_data")
    @patch("cnascorecard_pipeline.pipeline.generate_search_index")
    def test_generate_summary_files(
        self, mock_search_index, mock_chunked_data, mock_write_json, sample_config, sample_cna_outputs
    ):
    ...
    @patch("cnascorecard_pipeline.pipeline.write_json_file")
    @patch("cnascorecard_pipeline.pipeline.ensure_directory_exists")
    def test_generate_completeness_files(self, mock_ensure_dir, mock_write_json, sample_config):
        """Test completeness file generation."""
        pipeline = CNAScoreCardPipeline(sample_config)
        pipeline.field_utilization = [{"field": "test_field", "cna_percent": 75.0, "unique_cnas": 10}]
        pipeline.individual_cna_utilization = {"TestCNA": [{"field": "test_field", "percent": 100.0}]}
...
class TestPipelineMain:
    ...
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline")
    @patch("cnascorecard_pipeline.pipeline.setup_logging")
    @patch("sys.argv", ["cnascorecard_pipeline.pipeline.py", "--log-level", "DEBUG"])
    ...
    @patch("cnascorecard_pipeline.pipeline.CNAScoreCardPipeline")
    @patch("cnascorecard_pipeline.pipeline.setup_logging")
    @patch("sys.argv", ["cnascorecard_pipeline.pipeline.py"])
