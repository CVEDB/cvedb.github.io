...
from cnascorecard_pipeline.aggregation import aggregate_cna_scores
...
class TestAggregationFixtures:
    ...
    def sample_periods(self):
        ...
        return [(datetime(2024, 1, 1), datetime(2024, 1, 31)), (datetime(2024, 2, 1), datetime(2024, 2, 29))]
...
class TestAggregateCnaScores(TestAggregationFixtures):
    ...
    def test_aggregate_basic_functionality(
        self, mock_exists, mock_json_load, mock_file_open, sample_scored_cves, sample_periods, mock_cna_metadata
    ):
        ...
        for _cna_name, cna_result in result.items():
    ...
    def test_aggregate_score_calculations(
        self, mock_exists, mock_json_load, mock_file_open, sample_scored_cves, sample_periods
    ):
        """Test that score calculations are correct."""
        # Setup mocks
        mock_exists.return_value = True
        mock_json_load.return_value = [{"shortName": "TestCNA1"}, {"shortName": "TestCNA2"}]
    ...
    @patch("json.load")
    @patch("pathlib.Path.exists")
    def test_aggregate_percentage_calculations(self, mock_exists, mock_json_load, mock_file_open, sample_periods):
...
class TestTrendCalculations:
    ...
    def test_trend_calculation_called(self):
        ...
        sample_periods = [(datetime(2024, 1, 1), datetime(2024, 1, 31)), (datetime(2024, 2, 1), datetime(2024, 2, 29))]
...
class TestDataStructureValidation:
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("json.load")
    def test_output_structure_completeness(self, mock_json_load, mock_file, mock_exists):
        ...
        if "TestCNA" in result:
            ...
            required_scoring_fields = [
                ...
                "average_patchinfo",
                "percent_foundational_completeness",
                "percent_root_cause_analysis",
                "percent_software_identification",
                "percent_severity_and_impact",
                "percent_patchinfo",
                "trend_direction",
                "trend_description",
                "monthly_trends",
            ...
            required_cve_fields = ["cveId", "assigningCna", "datePublished", "totalCveScore", "scoreBreakdown"]
...
def test_aggregate_empty_input():
    ...
    periods = [(datetime(2025, 9, 1), datetime(2026, 3, 1)), (datetime(2025, 3, 1), datetime(2025, 8, 31))]
    with patch("pathlib.Path.exists", return_value=False):
